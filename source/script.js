...
function drawTrajectory(initX, initY) {
	...
	for(i = 0; i < 200; ++i) {
		...
		if (0.5*minScale >= Math.max(Math.abs(tempX),Math.abs(tempY))) {
		...
		} else if (50*minScale >= Math.max(Math.abs(tempX),Math.abs(tempY))) {
			ctx.strokeStyle = "#e6e600"; //Yellowish
		} else if (70*minScale >= Math.max(Math.abs(tempX),Math.abs(tempY))) {
